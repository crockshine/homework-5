...
import { renderTaskList } from '@/ui/renderTaskList';
...
// запросы
// CREATE
export async function createTask(event: SubmitEvent): Promise<void> {
...
const _getTaskList = async (params?: FetchTasksQueryParams): Promise<void> => {
  const tasks = await taskController.getAllTasks(params);
  renderTaskList(tasks);
};

async function _handleSearch(event: SubmitEvent): Promise<void> {
...
async function _handleSortByImportance(event: Event): Promise<void> {
...
async function _handleSortByCompleted(event: Event): Promise<void> {
  const target = event.target as HTMLInputElement;
  const isChecked = target.checked;
  if (params) {
    if (isChecked) {
...
export async function getUpdatedTaskList(result: boolean): Promise<void> {
  if (result) {
    await _getTaskList();
    alert('Успешно!');
    createForm?.reset();
...
export async function getTaskById(params: FetchTaskParams): Promise<void> {
...
// UPDATE
export async function completeTask(params: PatchTaskParams): Promise<void> {
  const task = await taskController.updateTaskById(params, { isCompleted: true });
  await getUpdatedTaskList(!!task);
}

export const updateTask = async (event: SubmitEvent, form: HTMLFormElement, task: Task): Promise<void> => {
  const formData = getFormFields(event, form);
  const updated = await taskController.updateTaskById({ id: task.id! }, { ...formData, isCompleted: task.isCompleted });
  await getUpdatedTaskList(!!updated);
};

// DELETE
export async function deleteTask(params: PatchTaskParams): Promise<void> {
